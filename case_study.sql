...
from products);
-- or
select product_id,product_name,max(price) as price
from products
group by 1,2
order by price desc
limit 1;
...
with cte as (select  o.customer_id as id , first_name as name,count(distinct i.product_id) as total_order from
orders o join customers c
on c.customer_id=o.customer_id
join order_items  i
on o.order_id=i.order_id
group by 1)
select id, name, total_order from cte
where total_order=3 ;
